...
class LocationService {
    ...
    startContinuousTracking() {
        ...
        // Backup interval-based tracking
        this.trackingInterval = setInterval(async () => {
            try {
                const position = await this.getCurrentPosition();
                this.handleLocationUpdate(position);
            } catch (error) {
                console.warn('⚠️ Interval position update failed:', error);
            }
    ...
    // Check if location should be saved (movement threshold)
    shouldSaveLocation(newLocation) {
        ...
        // Save if moved more than 100 meters or 5 minutes passed
    ...
    checkLocationTriggers(locationData) {
        ...
        if (locationData.speed !== null) {
            const isMoving = locationData.speed > 1; // > 1 m/s (3.6 km/h)
