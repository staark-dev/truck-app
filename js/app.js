// Driver Support App - Main Application Logic (Optimized Version)
class DriverApp {
constructor() {
...
setupLocationServiceIntegration() {
    // Connect LocationService with the main app
    if (this.locationService) {
        // Override updateDistanceDisplay to also update our UI elements
    ...
    if (this.locationService && typeof this.locationService.initialize === 'function') {
        try {
            ...
            if (initialized) {
                ...
                if (gpsStatus) {
                    gpsStatus.innerHTML = '<span class="status-dot status-gps"></span>GPS Ready';
                }
            } else {
                ...
                const gpsStatus = document.getElementById('gpsStatus');
                if (gpsStatus) {
                    gpsStatus.innerHTML = '<span class="status-dot status-offline"></span>GPS Offline';
                }
            }
        } catch (error) {
...
async reactivateGPS() {
    ...
    if (granted && this.locationService) {
        // Initialize LocationService again
...
restoreSession(sessionData) {
    ...
    try {
        ...
        // Start location tracking when program is restored
        if (this.locationService && typeof this.locationService.startTracking === 'function') {
            setTimeout(() => {
...
startProgram() {
    ...
    if (this.locationService && typeof this.locationService.startTracking === 'function') {
        setTimeout(() => {
            this.locationService.startTracking();
            console.log('📍 Location tracking started with program');
        }, 1000);
    }
...
endProgram() {
    ...
    if (this.locationService && typeof this.locationService.stopTracking === 'function') {
...
speakStatus() {
    ...
    if (this.locationService && typeof this.locationService.getTotalDistance === 'function') {
        totalDistance = this.locationService.getTotalDistance();
    }
...
loadGPSPage() {
    ...
    // Use LocationService for GPS data
    if (this.locationService) {
...
displayGPSData(locationData) {
    ...
    const distance = this.locationService ? this.locationService.getTotalDistance() : 0;
    ...
    gpsContent.innerHTML = `
        <div style="background: #e8f5e8; border-radius: 8px; padding: 15px; margin-bottom: 20px; border-left: 4px solid #4caf50;">
            ...
            <div style="margin-bottom: 5px;">📏 Distanță parcursă: <strong>${distance.toFixed(2)} km</strong></div>
        ...
        <div style="height: 200px; background: linear-gradient(135deg, #74b9ff, #0984e3); border-radius: 12px; display: flex; flex-direction: column; align-items: center; justify-content: center; color: white; font-weight: bold; margin-bottom: 20px; position: relative; overflow: hidden;">
            ...
            <div>HARTĂ GPS</div>
            <small style="opacity: 0.8;">Tracking activ: ${distance.toFixed(2)} km</small>
    ...
    // Start location updates for this page
...
// Reset distance function for GPS page
resetDistance() {
    if (this.locationService && typeof this.locationService.resetDistance === 'function') {
        this.locationService.resetDistance();
        this.showToast('🔄 Distanța a fost resetată');
        
        // Refresh GPS page if active
...
startLocationUpdates() {
    // Update location every 5 seconds when GPS tab is active
    if (this.locationUpdateInterval) {
        clearInterval(this.locationUpdateInterval);
    }

    this.locationUpdateInterval = setInterval(() => {
        if (this.currentTab === 'gps' && this.locationService && this.locationService.lastKnownPosition) {
...
updateGPSPageData() {
    if (this.locationService && this.locationService.lastKnownPosition) {
        const distance = this.locationService.getTotalDistance();
        
        // Update distance in GPS page
        const distanceElement = document.querySelector('#gpsContent div[style*="📏"]');
        if (distanceElement) {
            distanceElement.innerHTML = `📏 Distanță parcursă: <strong>${distance.toFixed(2)} km</strong>`;
        }
        
        // Update map subtitle
        const mapSubtitle = document.querySelector('#gpsContent small');
        if (mapSubtitle) {
            mapSubtitle.textContent = `Tracking activ: ${distance.toFixed(2)} km`;
        }
    }
}
...
loadReportsPage = () => {
    ...
    // Generate HTML for reports content
    ...
    if (reportsPage) {
        // ✅ re-render without destroying the node (keep position, .page class, etc.)
        reportsPage.innerHTML = reportsInner;
        return;
    }

    // ♻️ doesn't exist? create and insert at correct position in structure
    reportsPage = document.createElement('div');
    reportsPage.className = 'page';
    reportsPage.id = 'pageReports';
    reportsPage.innerHTML = reportsInner;

    // insert after #pageFuel, or before #pageSettings, otherwise at end as fallback
    const fuelPage = document.getElementById('pageFuel');
        ...
        <div class="card">
            ...
            <div style="background: #f8f9fa; border-radius: 8px; padding: 15px;">
                ...
                <div style="display: flex; justify-content: space-between; font-weight: bold; color: #3498db;">
                    <span>📏 Distanță:</span>
                    <span id="reportTotalDistance">${distance.toFixed(2)} km</span>
                </div>
            </div>
        </div>
    `;
        ...
        <div class="card">
            ...
            <div style="background: linear-gradient(135deg, #667eea, #764ba2); color: white; padding: 20px; border-radius: 12px; margin-bottom: 15px;">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 15px; text-align: center;">
                    ...
                    <div>
                        <div style="font-size: 18px; font-weight: bold;">${distance.toFixed(1)}km</div>
                        <div style="font-size: 12px; opacity: 0.9;">Parcursă</div>
                    </div>
                </div>
            </div>
        </div>
    ...
    return `
        ...
        <body>
            ...
            <div class="stats">
                ...
                <div class="stat-row">
                    <span>Distanță parcursă:</span>
                    <span>${distance.toFixed(2)} km</span>
                </div>
                <div class="stat-row">
                    <span><strong>Total timp:</strong></span>
...
async updateHomePageLocation() {
    try {
        // Use LocationService if available
        if (this.locationService && this.locationService.lastKnownPosition) {
            ...
            navigator.geolocation.getCurrentPosition(
                ...
                (error) => {
                    console.warn('⚠️ Failed to get home page location:', error);
...
if (app && typeof app.resetDistance === ‘function’) {
app.resetDistance();
}
}

function calculateRoute() {
if (app && typeof app.calculateRoute === ‘function’) {
app.calculateRoute();
}
}

function findParkingZones() {
if (app && typeof app.findParkingZones === ‘function’) {
app.findParkingZones();
}
}

function findGasStations() {
if (app && typeof app.findGasStations === ‘function’) {
app.findGasStations();
}
}

function shareLocation() {
if (app && typeof app.shareLocation === ‘function’) {
app.shareLocation();
}
}
...
if (app && typeof app.exportToPDF === ‘function’) {
app.exportToPDF();
}
}

function exportToExcel() {
if (app && typeof app.exportToExcel === ‘function’) {
app.exportToExcel();
}
}

function sendEmail() {
if (app && typeof app.sendEmail === ‘function’) {
app.sendEmail();
}
}

function printReport() {
if (app && typeof app.printReport === ‘function’) {
app.printReport();
}
}
...
if (app && typeof app.saveFuelData === ‘function’) {
app.saveFuelData();
}
}

// GPS reactivation function
function reactivateGPS() {
if (app && typeof app.reactivateGPS === ‘function’) {
return app.reactivateGPS();
}
}

// Permission reset function
function resetPermissions() {
if (app && typeof app.resetPermissions === ‘function’) {
app.resetPermissions();
}
}
...
if (app && typeof app.updateNetworkStatus === ‘function’) {
